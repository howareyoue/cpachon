...
public class MapsNaverActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private void enableLocationTracking() {
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED ||
            ...
            locationRequest.setFastestInterval(5000);  // 최소 5초마다 업데이트 요청
                        ...
                        if (directionsResponse != null && directionsResponse.route != null) {
                            ...
                            if (routes != null && !routes.isEmpty()) {
                                ...
                                for (List<Double> point : routes.get(0).path) {
                                    if (point.size() >= 2) { // 경로 포인트가 적어도 위도, 경도 두 값이 있는지 확인
                                        path.add(new LatLng(point.get(1), point.get(0))); // [위도, 경도] 순서로 변환
                                    }
                                }
                            ...
                            Toast.makeText(MapsNaverActivity.this, "잘못된 응답 데이터.", Toast.LENGTH_SHORT).show();
                        }
                    } else {
    ...
    // Directions API 서비스 정의
    public interface NaverDirectionsService {
        @GET("/map-direction/v1/driving")
        Call<DirectionsResponse> getWalkingRoute(
                @Query("X-NCP-APIGW-API-KEY-ID") String clientId,
                @Query("X-NCP-APIGW-API-KEY") String clientSecret,
